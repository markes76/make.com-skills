...
The npm distribution is a thin, zero-dependency Node bridge around the bundled AI skill and Python companion. It does not reimplement Make's API. The installed AI skill leads the engagement and an MCP-capable agent remains the schema-aware control plane; the terminal companion calls the official `make-cli` only for authentication and safe fallback evidence.
...
`skill install` copies the public AI skill to the selected client and refuses to replace an existing skill or adapter unless `--force` is explicit. `update` makes a read-only request to the npm registry, compares versions, and prints an install command only when a newer version exists. It never installs, updates, or runs a package by itself. Users can explicitly opt into a once-per-day notification on later `doctor` or legacy `wizard` runs with `make-com-skills notifications enable`; disable it with `make-com-skills notifications disable`.

`npx ...@latest` is the simplest choice for occasional skill installation: it resolves the current `latest` release when invoked. The npx bridge needs Node 18+; Python 3 is needed only when using the terminal companion. For a supported desktop platform, `make-com-skills make-cli install` explicitly displays and confirms the pinned official Make CLI release, verifies its SHA-256, and writes it to user-local storage. It does not run during npm install, alter `PATH`, or save credentials; the official CLI's own `login` flow remains responsible for authentication.
