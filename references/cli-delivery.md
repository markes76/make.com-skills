...
The official [`make-cli`](https://github.com/integromat/make-cli) is the API runtime. This project deliberately does **not** fork, reimplement, or shadow it. `make-skills` is an additive wizard and skill pack that calls the installed official binary.

## Boundary

- `make-cli`: authenticated Make API commands, generated from Make's supported API surface.
- `make-skills`: guided onboarding, safe review flows, local design handoffs, evaluation, and governed learning.
...
- `make-skills wizard` defaults to account/scenario reads. It can invoke the official `make-cli login` wizard only after the user chooses that step.
